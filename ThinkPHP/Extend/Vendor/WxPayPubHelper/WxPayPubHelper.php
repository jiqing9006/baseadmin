...
class Common_util_pub {
    ...
    function postXmlSSLCurl($xml, $url, $second = 30) {
        ...
        curl_setopt($ch, CURLOPT_SSLCERT, C('WxPayConf_pub.SSLCERT_PATH'));
        //默认格式为PEM，可以注释
        curl_setopt($ch, CURLOPT_SSLKEYTYPE, 'PEM');
        curl_setopt($ch, CURLOPT_SSLKEY, C('WxPayConf_pub.SSLKEY_PATH'));
        //post提交方式
        curl_setopt($ch, CURLOPT_POST, true);
        curl_setopt($ch, CURLOPT_POSTFIELDS, $xml);
        $data = curl_exec($ch);
...
class Wxpay_client_pub extends Common_util_pub {
     ...
     */
    function createXml() {
        $this->parameters["appid"] = C('WxPayConf_pub.APPID');//公众账号ID
        $this->parameters["mch_id"] = C('WxPayConf_pub.MCHID');//商户号
...
class UnifiedOrder_pub extends Wxpay_client_pub {
    function __construct() {
        ...
        //设置curl超时时间
        $this->curl_timeout = C('WxPayConf_pub.CURL_TIMEOUT');
    }
...
class NativeCall_pub extends Wxpay_server_pub {
    ...
    function createXml() {
        if ($this->returnParameters["return_code"] == "SUCCESS") {
            $this->returnParameters["appid"] = C('WxPayConf_pub.APPID');//公众账号ID
            $this->returnParameters["mch_id"] = C('WxPayConf_pub.MCHID');//商户号
...
class JsApi_pub extends Common_util_pub {
    ...
    function createOauthUrlForCode($redirectUrl) {
        $urlObj["appid"] = C('WxPayConf_pub.APPID');
    ...
    function createOauthUrlForOpenid() {
        $urlObj["appid"] = C('WxPayConf_pub.APPID');
        $urlObj["secret"] = C('WxPayConf_pub.APPSECRET');
    ...
    public function getParameters() {
        $jsApiObj["appId"] = C('WxPayConf_pub.APPID');
        $timeStamp = time();
