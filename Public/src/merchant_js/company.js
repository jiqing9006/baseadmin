...
$(".side-menu li:eq(0) ul li:eq(3)").addClass("current-page");
...
function change(nums,fileId,width,height){
    ...
    if(!myDate || myDates-myDate>5){
        myDate = myDates;
        $.ajaxFileUpload({
            url:'/admin.php/Annex/index/folder/bgimg?size=1&width='+width+'&height='+height,
            secureuri:false,
...
$("#add_form").ajaxForm({
    ...
    success : function(obj){
        ...
        if(obj.errno == 1){
            alert(obj.errdesc);
            window.location.href='/admin.php/Company/index';
        }else{
            alert(obj.errdesc);
        }
        return false;
