...
$(".side-menu li:eq(2) ul li:eq(1)").addClass("current-page");
...
function change(nums,fileId){
    ...
    if(!myDate || myDates-myDate>5){
        myDate = myDates;
        $.ajaxFileUpload({
            url:'/admin.php/Annex/index/folder/bgimg?size=1&width=750&height=350',
            secureuri:false,
...
$("#add_form").ajaxForm({
    ...
    success : function(obj){
        ...
        if(obj.errno == 1){
            alert(obj.errdesc);
            window.location.href='/admin.php/Topiclimit/index?page=' + page;
        }else{
            alert(obj.errdesc);
        }
        return false;
